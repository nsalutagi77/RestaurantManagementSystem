...
def placeOrder(request):
    to_email = []
    customers = Customer.objects.get(customer=request.user)
    ...
    for item in items:
        ...
        food = item.food
        print("food", food)
        order = Order.objects.create(customer=customers, order_timestamp=timezone.now(), payment_status="Pending", 
        delivery_status="Pending", total_amount=food.sale_price, payment_method="Cash On Delivery", location=customers.address)
        print("order = ",order)
        order.save()
    ...
    print("email = ",customers.customer.email)
    to = str(customers.customer.email)
    to_email.append(to)
    from_email = 'nsalutagi77@gmail.com'
    message = "Hi "+customers.customer.first_name+" Your order was placed successfully. Please go to your dashboard to see your order history."
    send_mail(
        mail_subject,
        message,
        from_email,
